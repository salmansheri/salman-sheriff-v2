      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed top-0 left-0 w-full h-screen bg-slate-950/80 backdrop-blur-lg backdrop-saturate-150 p-6 overflow-y-auto z-50"
          >
            ...
            <nav className="space-y-4 mt-10">
              {navItems.map((nav) => (
                <a
                  ...
                  className="flex relative items-center isolate justify-between border-t last:border-b py-8 group/nav-item text-slate-300 hover:text-violet-800 cursor-pointer"
                >
                  <span className="text-3xl group-hover/nav-item:pl-4 transition-all duration-500">
                    {nav.label}
                  </span>
                  <ArrowRight />
                  <div className="w-full absolute h-0 bg-slate-900/70 group-hover/nav-item:h-full transition-all duration-500 bottom-0 -z-10"></div>
                </a>
              ))}
            </nav>
          </motion.div>
